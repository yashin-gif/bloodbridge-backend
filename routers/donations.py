...
def create_donation(
    ...
    blood_request = db.query(BloodRequest).filter(
        BloodRequest.id == donation_data.blood_request_id
    ).first()
    ...
    # User cannot donate to their own blood request
    ...
    # Check if this user already donated for this request
    existing_donation = db.query(Donation).filter(
        Donation.donor_id == current_user.id,
        Donation.blood_request_id == donation_data.blood_request_id
    ).first()

    if existing_donation:
        raise HTTPException(
            status_code=400,
            detail="You have already submitted a donation for this blood request"
        )

    new_donation = Donation(
        donor_id=current_user.id,
        blood_request_id=donation_data.blood_request_id,
...
def get_donations(
    ...
    return db.query(Donation).offset(
        skip
    ).limit(
        limit
    ).all()
...
def update_donation(
    ...
    if (
        ...
        raise HTTPException(
            status_code=403,
            detail="You cannot update this donation"
        )
